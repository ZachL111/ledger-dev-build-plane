...
`ledger-dev-build-plane` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies build behavior through node-edge fixtures, with cycle and reachability reports and no production deployment claims.
...
The score is deterministic. The current threshold is 175. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 4.
